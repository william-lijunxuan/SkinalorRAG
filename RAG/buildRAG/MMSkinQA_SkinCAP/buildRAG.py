...
from config import CSV_PATH_MMSkinQA,CSV_PATH_SkinCAP, IMAGE_DIR_SkinCAP,IMAGE_DIR_MMSkinQA, CHROMA_PERSIST_PATH, CHROMA_COLLECTION_NAME, EMBEDDING_MODEL_NAME
...
def main():
    ...
    try:
        ...
        skincap_df = pd.read_csv(CSV_PATH_SkinCAP)
        with open(CSV_PATH_SkinCAP, "r", encoding="utf-8") as f:
        ...
        skincap_df = skincap_df.rename(columns={
            'skincap_file_path': 'image_file',
            'caption_zh_polish_en': 'text_description',
            'disease': 'disease_name'
        })[['image_file', 'text_description', 'disease_name']]

        skincap_df['image_dir'] = IMAGE_DIR_SkinCAP
        df = pd.concat([skincap_df, MMSkinQA_df], ignore_index=True)


        df['image_file'] = (df['image_file'].str.replace('^dataset/', '', regex=True))
        ...
        filtered_df = df.dropna(subset=['text_description'])
        filtered_df = filtered_df[filtered_df.apply(valid_row, axis=1)].reset_index(drop=True)


        # print(filtered_df.head())
        # filtered_df.to_csv("combined_output.csv", index=False)
        print(f"Successfully loaded {len(filtered_df)} case records from {CSV_PATH_MMSkinQA} and {CSV_PATH_SkinCAP}.")
    except FileNotFoundError:
        print(f"Error: CSV file not found at the specified path {CSV_PATH_MMSkinQA} and {CSV_PATH_SkinCAP}")
        return
    ...
    for index, row in tqdm(filtered_df.iterrows(), total=filtered_df.shape[0], desc="Processing case..."):
        case_id = str(index)
        ...
        if not os.path.exists(image_full_path):
            print(f"os.path.exists:{image_full_path}")
            continue
